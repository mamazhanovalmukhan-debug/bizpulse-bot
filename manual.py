"""POS-адаптер iiko — STUB — подключить API: реализовать реальный API."""
from services.pos.base import POSAdapter
import logging

class IikoAdapter(POSAdapter):
    async def test_connection(self) -> bool:
        logging.warning("iiko adapter not implemented yet")
        return False

    async def sync_transactions(self, from_dt, to_dt) -> list:
        # STUB — подключить API: реализовать запрос к API iiko
        return []
