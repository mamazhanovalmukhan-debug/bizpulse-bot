...
async def create_plan_invoice(bot: Bot, chat_id: int,
    ...
    ENTERPRISE — не создаёт invoice, отправляет на поддержку.
    """
    ...
    if plan_code == "ENTERPRISE" or plan["price"] == 0:
        await bot.send_message(
            chat_id,
            "Enterprise — тариф с индивидуальной ценой.\n\n"
            f"Напишите в поддержку: {config.SUPPORT_USERNAME}\n"
            "Укажите: название бизнеса и нужное количество точек."
        )
        return False
...
async def validate_pre_checkout(payload: str, amount: int,
    ...
    if amount != plan["price"]:
        return {
            "ok": False,
            "error": f"Неверная сумма. Ожидалось {plan['price']}, получено {amount}."
        }
