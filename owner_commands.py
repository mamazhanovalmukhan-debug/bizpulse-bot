...
from ai_analyst import analyze_day, weekly_advice, analyze_stock
...
async def owner_start(message: Message):
    await message.answer("BizPulse — твой управляющий\n\nВыбери действие:", reply_markup=owner_keyboard())
...
async def status_now(message: Message):
    ...
    await message.answer(
        ...
        f"Открытие: {'заполнено' if s['opened'] else 'нет отчета'}\n"
        f"Закрытие: {'заполнено' if s['closed'] else 'смена идет'}\n\n"
        f"Выручка за день\n"
        f"Наличные: {fmt(s['total_cash'])}\n"
        f"Безнал:   {fmt(s['total_card'])}\n"
        f"Итого:    {fmt(s['total'])}\n\n"
        f"Отчетов сегодня: {s['reports_count']}"
    )
...
async def today_results(message: Message):
    ...
    if not reports:
        await message.answer("За сегодня отчетов ещё нет.")
        return
    lines = []
    for r in reports:
        ...
        if revenue:
            line += f" — {fmt(revenue)}"
        lines.append(line)
    await message.answer("ОТЧЕТЫ ЗА СЕГОДНЯ\n\n" + "\n".join(lines))
...
async def competitor_intel(message: Message):
    await message.answer("Читаю посты конкурентов...")
    raw = await get_raw_competitor_posts()
    await message.answer(raw)
    await message.answer("Формирую совет...")
