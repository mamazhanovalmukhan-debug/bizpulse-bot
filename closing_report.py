...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if summary["closed"]:
        await message.answer("Смена уже закрыта сегодня. Хорошего отдыха!", reply_markup=employee_keyboard())
        return
    ...
    await message.answer(
        "Закрытие смены\n\n"
        "Шаг 1 из 5\n"
        "Сколько наличных ты принял за день (выручка)?\n"
        "Это деньги от покупателей — не считай остаток, который был утром.\n"
        "Только цифры:",
        reply_markup=kb_cancel(),
    )
...
async def step_expenses(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Записал.\n\n"
        f"Шаг 4 из 5\nСколько наличных сейчас в кассе (остаток)?\n"
...
async def step_comment(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.answer("Отчет отправлен! Хорошего отдыха\n\n" + text, reply_markup=employee_keyboard())
    try:
