...
async def ask_groq(prompt: str, system: str = "") -> str:
    if not GROQ_KEY:
        return "⚠️ GROQ_API_KEY не настроен в переменных окружения Railway."
    messages = []
    if system:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if "choices" not in data:
                logging.error(f"Groq ответ без choices: {data}")
                return f"⚠️ Ошибка ИИ: {data.get('error', {}).get('message', 'неизвестная ошибка')}"
